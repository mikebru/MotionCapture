...
public class SteamVR_ControllerManager : MonoBehaviour
{
	...
	[Tooltip("Set to true if you want trackers to be assigned to virtual objects with matching serial numbers")]
	public bool assignBySerialNumber;
	...
	private void SetTrackedDeviceIndex(int objectIndex, uint trackedDeviceIndex)
	{
		...
		if (trackedDeviceIndex != indices[objectIndex])
		{
			...
			//ADDED BY MICHAEL BRUNER
			//Method for making virtual objects and physical objects match constitently
			if (assignBySerialNumber == true)
			{
				ETrackedPropertyError error = new ETrackedPropertyError();
				StringBuilder sb = new StringBuilder();
				OpenVR.System.GetStringTrackedDeviceProperty(trackedDeviceIndex, ETrackedDeviceProperty.Prop_SerialNumber_String, sb, OpenVR.k_unMaxPropertyStringSize, ref error);
				var probablyUniqueDeviceSerial = sb.ToString();

				Debug.Log("Index: " + trackedDeviceIndex.ToString());
				Debug.Log("Serial: " + probablyUniqueDeviceSerial);

				if (obj.GetComponent<TrackerSerial>() != null)
				{
					//if the virtual objects serial doesn't match the real serial find the virtual object that does
					if (obj.GetComponent<TrackerSerial>().SerialNumber != probablyUniqueDeviceSerial)
					{
						for (int i = 0; i < objects.Length; i++)
						{
							if (objects[i].GetComponent<TrackerSerial>().SerialNumber == probablyUniqueDeviceSerial)
							{
								obj = objects[i];
								break;
							}
